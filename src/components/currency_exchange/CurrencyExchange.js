...
export default function CurrencyExchange({currency, isListLoading}) {
	...
	const swapCurrency = () => {
	  ...
	  useEffect( () => {
		  setEnteredValue(1)
		  setLoading(true);
	    fetch(`https://currency-exchange.p.rapidapi.com/exchange?q=1.0&from=${currencyFrom}&to=${currencyTo}`, {
	          "method": "GET",
	          "headers": {
	        ...
	        .then(response => {setExchangeValue(response);setLoading(false);})
	        .catch(err => {
	          console.log(err);
	        }
	    );
				...
				<div className='row'>
					...
					<div className='col s12 l5 right'>
						...
						<div className='input-field'>
							...
							>
								{currency.map((data) => <option key={data} value={data}>{data}</option>)}
							</select>
						</div>
